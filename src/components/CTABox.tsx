...
export function CTABox({ title, text, kind }: { title: string; text: string; kind: 'download' | 'checklist' }): JSX.Element {
	...
	return (
		<div className="p-5 rounded-2xl border border-line bg-panel">
			...
			<a className={`inline-flex mt-3 h-10 px-4 items-center rounded-xl ${cls}`} href="#">{kind === 'download' ? 'Baixar' : 'Ver checklist'}</a>
		</div>
	);
}
